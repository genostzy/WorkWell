-- An HR account no longer carries a private plane of its own — that is
-- the whole point of 0024. But provision_person and invite_person still
-- unconditionally inserted the 'employee' role before conditionally
-- adding 'hr', so a p_is_hr = true call left the account holding both
-- roles anyway: the exact two-planes-on-one-account shape 0024 exists to
-- retire. An account is one or the other now, never both.
